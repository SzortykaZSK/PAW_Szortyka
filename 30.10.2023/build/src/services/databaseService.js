...
connection.connect((err) => {
    ...
    connection.query('CREATE DATABASE IF NOT EXISTS testDatabase', (err, result) => {
        if (err)
            throw err;
        console.log('Database created or already exists');
    });
    connection.query('USE testDatabase', (err, result) => {
        if (err)
            throw err;
        console.log('Database Selected');
    });
    connection.query('CREATE TABLE IF NOT EXISTS students (id INT KEY AUTO_INCREMENT, name VARCHAR(255), surname VARCHAR(255), email VARCHAR(255))', (err, result) => {
        if (err)
            throw err;
        ...
        connection.query(checkTableQuery, function (error, results) {
            if (error) {
                ...
                if (results.length > 0) {
                    ...
                    connection.query(sql, [values1], (err, result) => {
                        if (err)
                            throw err;
        ...
        connection.query('CREATE TABLE IF NOT EXISTS formData (id INT KEY AUTO_INCREMENT, name VARCHAR(255), email VARCHAR(255), theme VARCHAR(255), content TEXT)', (err, result) => {
            if (err)
                throw err;
            console.log('Table "formData" created or already exists');
        });
        connection.query('CREATE TABLE IF NOT EXISTS subjects (id INT KEY AUTO_INCREMENT, name VARCHAR(255), hoursAWeek INT)', (err, result) => {
            if (err)
                throw err;
            ...
            connection.query(checkTableQuery, function (error, results) {
                if (error) {
                    ...
                    if (results.length > 0) {
                        ...
                        connection.query(sql, [values2], (err, result) => {
                            if (err)
                                throw err;
