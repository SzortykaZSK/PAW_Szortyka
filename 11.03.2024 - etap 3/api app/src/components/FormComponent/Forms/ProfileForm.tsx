...
function ProfileForm() {
  ...
  const handleSubmit = async(e: React.FormEvent<HTMLFormElement>) =>{
    ...
    if(httpMethod == 'post'){
        ...
        try {
            const response = await axios.patch('http://localhost:3000/profile/'+formDataId, formData);
            console.log('Response:', response.data);
          } catch (error) {
            console.error('Error:', error);
          }
        ...
        try {
            const response = await axios.delete('http://localhost:3000/profile/'+formDataId);
            console.log('Response:', response.data);
          } catch (error) {
            console.error('Error:', error);
          }
  ...
  return (
    <form onSubmit={handleSubmit}>
        ...
        {httpMethod == 'post'? <>
            <input type="number" name="userId" value={formData.userId} onChange={handleChange} placeholder="userID"/>
